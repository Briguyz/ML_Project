...
## test data
melt_test4 <- melt(melt_test3, id = c('budget','revenue','runtime','vote_average','vote_count','net.revenue','ratio','ratio_indicator','studio','value','studio_unclass','actor','value2','actor_unclass','genre','value3','genre_unclass')) 

names(melt_test4)[19] = "value4" # rename 19th col studio

melt_test4 = melt_test4[melt_test4$value4 ==TRUE, ] # takes out duplicates by getting rid of all the FALSE values
names(melt_test4)[18] = "languange" # rename 18nd col studio

languange_vector_test <-unclass(melt_test4$languange)

melt_test4$languange_unclass <- languange_vector_test
```
...
## test
svm_data3 <- subset(melt_test4, select = -c(value, value2, value3, value4))
svm_data4 <- subset(melt_test4, select = -c(studio, value, actor, value2, genre, value3, languange, value4))
```
...
data <- write_excel_csv(svm_data1, "svm_testdata_labeled.csv")
data <- write_excel_csv(svm_data2, "svm_testdata.csv")
```
